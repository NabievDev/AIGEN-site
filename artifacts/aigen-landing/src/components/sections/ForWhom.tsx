...
export function ForWhom() {
  ...
  const leftCardY = useTransform(scrollYProgress, [0, 1], [100, -60]);
  const rightCardY = useTransform(scrollYProgress, [0, 1], [160, -100]);

  return (
    <section id="about" className="relative py-24 md:py-32 bg-white overflow-hidden">
      <motion.div
        style={{ y: blobY, background: "radial-gradient(circle, rgba(124,58,237,0.12) 0%, rgba(6,182,212,0.08) 50%, transparent 70%)" }}
      ...
      <div ref={ref} className="container mx-auto px-4 md:px-6 max-w-6xl relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-8 mb-10 items-start">
          {/* Kids Card — slower */}
          <motion.div style={{ y: leftCardY }} className="group relative">
            ...
            <div className="relative bg-white border border-gray-200/80 rounded-3xl p-8 md:p-10 shadow-[0_4px_30px_-12px_rgba(17,24,39,0.08)] group-hover:shadow-[0_20px_60px_-20px_rgba(124,58,237,0.35)] group-hover:-translate-y-1.5 group-hover:border-transparent transition-all duration-500 overflow-hidden">
              ...
              <ul className="relative space-y-3">
                {kidPoints.map((text, i) => (
                  <motion.li key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.2 + i * 0.08 }} className="flex items-start gap-3">
                    <div className="shrink-0 mt-0.5 w-6 h-6 rounded-lg bg-[#7C3AED]/10 flex items-center justify-center">
                      <span className="font-mono text-[10px] font-bold text-[#7C3AED]">{String(i + 1).padStart(2, "0")}</span>
                    </div>
          ...
          {/* Parents Card — faster */}
          <motion.div style={{ y: rightCardY }} className="group relative">
            ...
            <div className="relative bg-white border border-gray-200/80 rounded-3xl p-8 md:p-10 shadow-[0_4px_30px_-12px_rgba(17,24,39,0.08)] group-hover:shadow-[0_20px_60px_-20px_rgba(6,182,212,0.35)] group-hover:-translate-y-1.5 group-hover:border-transparent transition-all duration-500 overflow-hidden">
              ...
              <ul className="relative space-y-3">
                {parentPoints.map((text, i) => (
                  <motion.li key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 + i * 0.08 }} className="flex items-start gap-3">
                    <div className="shrink-0 mt-0.5 w-6 h-6 rounded-lg bg-[#06B6D4]/10 flex items-center justify-center">
                      <span className="font-mono text-[10px] font-bold text-[#06B6D4]">{String(i + 1).padStart(2, "0")}</span>
                    </div>
                    <span className="text-gray-700 leading-relaxed text-[15px] md:text-base pt-0.5">{text}</span>
                  </motion.li>
                ))}
        ...
        >
          ...
          <div className="relative flex flex-col sm:flex-row items-center gap-5 px-5 sm:px-8 py-6 rounded-3xl bg-gradient-to-br from-[#2E1065] via-[#4C1D95] to-[#5B21B6] border border-[#A855F7]/30 shadow-2xl shadow-[#7C3AED]/25 overflow-hidden">
            ...
            <div className="pointer-events-none absolute inset-0 opacity-[0.07] mix-blend-screen" style={{ backgroundImage: "radial-gradient(circle, #ffffff 1px, transparent 1px)", backgroundSize: "16px 16px" }} />
            ...
            <div className="relative flex-1 text-center sm:text-left">
              ...
              <p className="text-white text-sm md:text-base lg:text-lg font-medium leading-snug">
                Опыт программирования <span className="text-white/65">не нужен.</span> Нужен только компьютер <span className="text-white/65">и</span> интерес.
              </p>
            </div>
